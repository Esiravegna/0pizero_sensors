...
class MQTTClient(object):
	...
	def __init__(self, server=settings['MQTT_SERVER'], topic=settings['MQTT_TOPIC'], 
			port=settings['MQTT_PORT']):
		self.mqttc = mqtt.Client(MQTT_CLIENT_ID, clean_session=MQTT_CLEAN_SESSION)
		self.port = port
		self.host = host
	...
	def on_mqtt_connect(self, mosq, obj, flags, result_code):
	    ...
	    if result_code == 0:
	        ...
	        mqttc.publish(self.lwt, "1", qos=0, retain=True)
	    elif result_code == 1:
	...
	def publish(self, subtopic, payload):
		...
		if not self.is_connected:
			...
			mqttc.publish("{}/{}".format(self.topic, subtopic), payload, qos=0, retain=False)
			result = True
		return result
